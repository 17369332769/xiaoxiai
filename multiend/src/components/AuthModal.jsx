...
export default function AuthModal({
  ...
  return (
    <View className="modal-overlay" onClick={handleClose}>
      <View className="modal-content" onClick={(e) => e.stopPropagation()} style={{ maxWidth: '420px' }}>
        <View className="modal-header">
          <View className="modal-title"><Text>👤</Text><Text>账号中心 (Account)</Text></View>
          <View className="close-btn" onClick={handleClose}>&times;</View>
        </View>

        {account?.bound ? (
          <View style={{ textAlign: 'center', padding: '12px 0' }}>
            <View style={{ fontSize: '40px', marginBottom: '8px' }}>💞</View>
            <View style={{ color: 'var(--text-pink)', fontWeight: 600, marginBottom: '6px' }}>
              已绑定账号
            </View>
            <View style={{ color: 'var(--text-muted)', fontSize: '13px', marginBottom: '18px' }}>
              {account.identifier}
            </View>
            <View style={{ color: 'var(--text-muted)', fontSize: '12px', marginBottom: '18px' }}>
            ...
            <View style={{ marginTop: '18px', paddingTop: '14px', borderTop: '1px solid rgba(255, 255, 255, 0.08)', textAlign: 'left' }}>
              <View style={{ fontSize: '13px', color: 'var(--text-pink)', fontWeight: 600, marginBottom: '6px' }}>
                数据与隐私
              </View>
              <View style={{ fontSize: '11px', color: 'var(--text-muted)', marginBottom: '12px', lineHeight: 1.6 }}>
                你可以随时导出自己的全部数据，或永久注销账号。注销会删除你的资料、聊天、记忆、账单等所有记录，且无法恢复。
              </View>
              <View
                className="btn-secondary"
                style={{ width: '100%', padding: '10px', marginBottom: '10px' }}
                onClick={dataBusy ? undefined : handleExport}
              >
                {dataBusy ? '处理中...' : '导出我的数据（JSON）'}
              </View>
              {confirmingDelete && (
                <View style={{ fontSize: '12px', color: 'var(--accent-gold)', marginBottom: '10px', lineHeight: 1.5 }}>
              ...
              <View
                ...
                onClick={dataBusy ? undefined : handleDelete}
              >
                {dataBusy ? '处理中...' : (confirmingDelete ? '确认永久注销账号' : '注销账号')}
              </View>

              <View style={{ fontSize: '11px', color: 'var(--text-muted)', marginTop: '12px' }}>
                <Text>
                  <Text style={{ color: 'var(--text-pink)' }}>隐私政策</Text>
                  {' · '}
                  <Text style={{ color: 'var(--text-pink)' }}>服务条款</Text>
                </Text>
              </View>
            </View>
          </View>
          ...
          <View>
            <View style={{ fontSize: '13px', color: 'var(--text-pink)', fontWeight: 600, marginBottom: '4px' }}>
              重置密码
            </View>
            <View style={{ fontSize: '11px', color: 'var(--text-muted)', marginBottom: '12px', lineHeight: 1.6 }}>
            ...
            <View className="form-field">
              ...
              <View style={{ display: 'flex', gap: '8px' }}>
                ...
                <View
                  ...
                  onClick={codeSending ? undefined : () => handleSendCode('reset')}
                >
            ...
            <View
              ...
              onClick={submitting ? undefined : handleResetPassword}
            >
          ...
          <View>
            <View className="auth-tab-row">
              <View className="btn-secondary" style={tabButtonStyle(tab === 'login')} onClick={authPending ? undefined : () => switchTab('login')}>
                登录
              </View>
              <View className="btn-secondary" style={tabButtonStyle(tab === 'register')} onClick={authPending ? undefined : () => switchTab('register')}>
                注册并绑定
              </View>
            </View>
              ...
              <View className="form-field">
                ...
                <View style={{ display: 'flex', gap: '8px' }}>
                  ...
                  <View
                    className="btn-secondary"
                    style={{ whiteSpace: 'nowrap', padding: '0 12px' }}
                    onClick={codeSending ? undefined : () => handleSendCode('register')}
                  >
                    {codeSending ? '发送中...' : (codeSent ? '重新发送' : '发送验证码')}
                  </View>
                </View>
            ...
            {needsLoginConfirm && confirmingLogin && (
              <View style={{ fontSize: '12px', color: 'var(--accent-gold)', marginBottom: '12px', lineHeight: 1.5 }}>
            ...
            <View
              className="btn-primary"
              style={{ width: '100%', padding: '11px', fontSize: '14px' }}
              onClick={authPending ? undefined : handleSubmit}
            >
              {authPending
                ? '处理中...'
                : tab === 'register'
